...
void TIMER0_Init(void)
{
	...
	TMOD|=0b_0000_0101; // Timer/Counter 0 used as a 16-bit timer
...
void main (void)
{
	...
	float frequency;
	//float period;
	volatile float V[4]; //v0=period, v1=frequency, v2=pulse, v3=phase
	...
	while(1)
	{
		...
		//printf("V0=%5.3f, V1=%5.3f, V2=%5.3f, V3=%5.3f\r", V[0], V[1], V[2], V[3]);

		if(Vmax<Volts_at_Pin(LQFP32_MUX_P2_2) && check==0)
		{
			Vmax=Volts_at_Pin(LQFP32_MUX_P2_2);
			if (Vmax==Volts_at_Pin(LQFP32_MUX_P2_2))
				check==1;
		}
		...
		//printf("f=%f Hz\r", frequency);
		printf("V0=%5.3f, V1=%5.3f, V2=%5.3f, V3=%5.3f f=%.3f Hz\r", V[0], V[1], V[2], V[3], frequency);		
		waitms(100);		
	}
}