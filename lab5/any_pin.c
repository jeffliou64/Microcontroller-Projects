...
//		V[3]=Volts_at_Pin(LQFP32_MUX_P2_3);
		...
		while(i<10)
		{
			...
			period1=(overflow_count*65536.0+TH0*256.0+TL0)*(12000.0/SYSCLK);
			if(period1>period2)
			{
				period2 = period1;
			}
		...
		TR0=0; // Stop timer 0, the 24-bit number [overflow_count-TH0-TL0] has the period!
		timediff=(overflow_count*65536.0+TH0*256.0+TL0)*(12000.0/SYSCLK);
		if (timediff>(period2/2))
		{
			phase=(omega*timediff);
		}
		else
		{
			phase=(-1)*(omega*timediff);
		}
		
		// Send the period to the serial port
		printf("\rT1=%5.3fs, v1rms:%5.3f V, v2rms:%5.3f V, phase=%5.3f", period2, vrrms, vtrms, phase);
		waitms(10);
	}
}