...
void main (void)
{
	...
	while(1)
	{
	voltage = Volts_at_Pin(LQFP32_MUX_P1_7);
	PWM_1 = 0;
	PWM_2 = 0;
	printf(GOTO_YX, 1,1);
	printf("\rChoose between two options: temperature controlled motor (1), or user-controlled motor (other): ");
	scanf("%d", selection);
	
	if(selection==1)
	{
		printf("\rvoltage: %.3f", voltage);
		printf("\nType 'o' and press enter to end PWM: ");
		scanf("%s", &proceed);
	}
	else{
	printf("\rPlease Input PWM Range 100 to -100: ");
	scanf("%d", &DRV);
	if(DRV < -100 || DRV > 100)
